...
Version 0.1.2 adds collision-resistant default filenames: a short context-derived prefix, local timestamp, and the `session_export.md` suffix. Explicit user-supplied filenames and paths remain authoritative. The plugin has no MCP server, authentication, external API, UI, analytics, or telemetry. It does not upload or transmit generated exports. Reviewers need no test account or fixture data and can run all supplied cases in a disposable conversation.
