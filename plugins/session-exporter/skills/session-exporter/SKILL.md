...
7. Unless the user specifies another filename or path, apply the context-derived naming rule in [references/export-spec.md](references/export-spec.md): use the stable `session_export` base, the local export timestamp, and a short filesystem-safe context suffix, such as `session_export__20260916-1430__plugin-publish-review.md`. Prefer the environment's designated user-facing output directory, then the current workspace. If filesystem output is unavailable, create a downloadable Markdown artifact using the product's supported artifact mechanism.
