...
7. Unless the user specifies another filename or path, apply the context-derived naming rule in [references/export-spec.md](references/export-spec.md): use a short filesystem-safe context prefix followed by the local export timestamp and the `session_export.md` suffix, such as `plugin-publish-review__20260916-1430__session_export.md`. Prefer the environment's designated user-facing output directory, then the current workspace. If filesystem output is unavailable, create a downloadable Markdown artifact using the product's supported artifact mechanism.
