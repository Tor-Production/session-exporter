...
Create one faithful Markdown artifact that another AI or human can use to continue, audit, or reconstruct the current conversation.
...
5. Build the document using the required structure and transcript rules in the reference. Preserve exact visible wording, code blocks, commands, URLs, errors, corrections, and relevant visible tool output whenever accessible.
...
7. Save the result as `session_export.md` unless the user specifies another filename. Prefer the environment's designated user-facing output directory, then the current workspace. If filesystem output is unavailable, create a downloadable Markdown artifact using the product's supported artifact mechanism.
